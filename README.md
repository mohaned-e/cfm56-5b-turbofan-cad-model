...
Built over roughly 48-72 hours as a single multi-body part with 176 features, modelling a simplified alternating rotor/stator axial architecture. Note: this deliberately does not match the real CFM56-5B stage-for-stage, actual configuration is a 36-blade fan with a 4-stage LPC and 9-stage HPC; this model uses a simplified 22-blade fan, 3-stage LPC, and 6-stage HPC, chosen to keep the build scope manageable while preserving the real architecture pattern (alternating rotor/stator, tip shrouds, variable stator vanes).

Section	Rotor blade stages	Stator vane stages
Fan	22 blades (real engine: 36)	—
...
This is a static geometric representation, not a functional mechanism. Overall proportions (fan diameter, engine length) are based on published CFM56-5B specifications. Stage counts for the fan, LPC, and HPC were deliberately simplified relative to the real engine (see table above) to keep build scope manageable within the project timeframe; the alternating rotor/stator pattern, variable stator vane mechanism, and turbine tip shrouds were modelled to reflect real engine architecture regardless of the simplified count. Dimensions not publicly available were estimated proportionally from the engine's own technical diagrams.
